				...
				if (!CheckingUserInputIssues.checkingLackInformation(textField)) {
					...
					if (result == 0) {
						...
						if (CheckingUserInputIssues.checkingPhoneAndAge(textField[2], textField[3])) {
							...
							if (mainInterface.searching(modelAuthor, clientMainFrame.tableChosen) != null) {
								JOptionPane.showMessageDialog(this, "Tác giả đã có id tồn tại trong danh sách", "Lỗi!!",
										JOptionPane.ERROR_MESSAGE);
							} else {
								result = mainInterface.add((Object) modelAuthor, clientMainFrame.tableChosen);
							...
							JOptionPane.showMessageDialog(this, "Tuổi và SĐT không hợp lệ!", "Lỗi!!",
									JOptionPane.ERROR_MESSAGE);
						}
					}
				}
