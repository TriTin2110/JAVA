...
public class Student_Manager_LL {
	static Scanner input = new Scanner(System.in);
	public void InputStudentList(LinkedList<Student> list)
	{
	...
	public void OutputStudentList(LinkedList<Student> list)
	{
	...
	public int SearchbyID(LinkedList<Student> list,String id)
	{
		for (int i=0;i<list.size();i++) if (list.get(i).getID().compareTo(id)==0) return i;
		return -1;
	}
	public int SearchbyName(LinkedList<Student> list,String name)
	{
		for (int i=0;i<list.size();i++) if (list.get(i).getName().compareTo(name)==0) return i;
		return -1;
	}
	public void SearchStudent(LinkedList<Student> list)
	{
	...
	public void Sort(LinkedList<Student> list)
	{
...
public void SortbyResult(LinkedList<Student> list)
	{
	...
	public void SortbyName(LinkedList<Student> list)
	{
		for (int i = 0; i<list.size(); i++) {
			for (int j =i+1;j<list.size();j++)
			{
	...
	public void deleteStudent(LinkedList<Student> list)
	{
